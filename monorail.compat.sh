...
# brush
{
	...
	if [ -z "$_MR_DIR" ]; then
		if [ "$XDG_DATA_HOME" ]; then
			_MR_DIR="$XDG_DATA_HOME/monorail"
		else
			_MR_DIR="$HOME/.local/share/data/monorail"
		fi
	fi
	if [ -z "$_MR_CONFIG" ]; then
	...
	*)
	...
	if [ ! -f "$_MR_CONFIG"/colors-"$_MR_HOSTNAME".sh ]; then
		cat "$_MR_DIR"/gradients/Default.sh "$_MR_DIR"/colors/Default.sh >"$_MR_CONFIG"/colors-"$_MR_HOSTNAME".sh
	fi
	...
	"xterm"* | "tmux"* | "screen"* | "alacritty"* | "rio" | "rxvt-unicode"*)
		...
		case "$_MR_LANG" in
		*.UTF-8)
			_MR_ELIPSIS=$(printf '\342\200\246')
			# UTF-8 "Lower one eighth block"
			_MR_LINE_SEGMENT=$(printf '\342\226\201')
			;;
		esac
		;;
	"Eterm" | "rxvt"*)
	...
	"wyse60" | "wy60" | "wy50" | "wy160")
		_MR_OFFSET=1
		_MR_REVERSE="$_MR_PREHIDE${ESC}G4$_MR_POSTHIDE"
		_MR_NORMAL="$_MR_PREHIDE${ESC}G0$_MR_POSTHIDE"
		bind 'set enable-bracketed-paste off'
		_MR_DUMB_TERMINAL=1
		;;
	*)
	...
	if [ "$_MR_XTERM_TERMINAL" ] || [ "$_MR_ANSI_TERMINAL" ]; then
		_MR_REVERSE="$_MR_PREHIDE${ESC}[7m$_MR_POSTHIDE"
	...
	_MR_SHOW_GRADIENT_PROMPT() {
		if [ "$KSH_VERSION" ] ||
			[ "$BASH_VERSION" ] ||
			[ "$ZSH_VERSION" ]; then
			if [ "$_MR_TRUECOLOR_TERMINAL" ]; then
	...
	_MR_GRADIENT_PROMPT() {
		...
		while [ "$I" -lt "$LINE_WIDTH" ]; do
			...
			# shellcheck disable=SC3054 # ksh
			_MR_LINE="$_MR_LINE${ESC}[38;2;${_PROMPT_LUT[$((${#_PROMPT_LUT[*]} * I / $((COLUMNS + 1))))]}m$_MR_LINE_SEGMENT"

			I=$((I + 1))
		done
		I=0
		unset _MR_TEXT_ARRAY
		while [ $I -lt "${#_MR_TEXT_ARRAY[*]}" ]; do
			_MR_TEXT_ARRAY[I]=""
			I=$((I + 1))
		done
		...
		if [ "$ZSH_NAME" ]; then
			...
			I=0
			while [ $I -lt "${#_MR_TEXT}" ]; do
				_MR_TEXT_ARRAY[I]=$(echo "$_MR_TEXT" | cut -c$((I + 1)))
				I=$((I + 1))
			done
		else
			I=0
		...
		while [ "$I" -lt "${_MR_TEXT_ARRAY_LEN}" ]; do
			...
			# shellcheck disable=SC3054 # ksh
			_MR_TEXT_FORMATTED="$_MR_TEXT_FORMATTED$_MR_PREHIDE${ESC}[0m${ESC}[48;2;${_PROMPT_LUT[LUT]}m${ESC}[38;2;${_PROMPT_TEXT_LUT[TEXT_LUT]}m$_MR_POSTHIDE${_MR_TEXT_ARRAY[I]}"
			I=$((I + 1))
		done
		# shellcheck disable=SC3054 # ksh
		RGB_CUR_COLOR=${_PROMPT_LUT[$((${#_PROMPT_LUT[*]} * $((_MR_TEXT_ARRAY_LEN + 1)) / $((COLUMNS + 1))))]}
		...
		# shellcheck disable=SC3054 # ksh
		[ ${#_PROMPT_LUT[*]} = 0 ] && HEX_CUR_COLOR="${_COLORS[21]}"
		_MR_CUR="${ESC}]12;#${HEX_CUR_COLOR}${BEL}"
	...
	_MR_UPDATE() {
		...
		if [ "$HOME" = "$PWD" ]; then
			...
			TITLE=$_MR_PWD_BASENAME
		fi
		if [ "$_MR_XTERM_TERMINAL" ]; then
		...
		if _MR_SHOW_GRADIENT_PROMPT; then
			...
			while [ "$I" -lt "$LINE_WIDTH" ]; do
				_MR_LINE="$_MR_LINE$_MR_LINE_SEGMENT"
				I=$((I + 1))
			done
			_MR_TEXT_FORMATTED="$_MR_TEXT"
		fi
		if [ "$_MR_XTERM_TERMINAL" ]; then
			_MR_LINE="$_MR_LINE$ESC(1"
		elif [ "$_MR_VTXXX_TERMINAL" ]; then
			_MR_LINE="$_MR_LINE$ESC(B${ESC}[7m"
		fi

		PS1="$_MR_TITLE$_MR_CUR$_MR_LINE
$_MR_REVERSE$_MR_TEXT_FORMATTED$_MR_NORMAL "
	...
	trap "_MR_UPDATE" WINCH

	# __git_ps1 ksh compatible with version shipped in git
	__git_ps1() {
		__GIT_PS1_REV=$(basename "$($_MR_GIT_BIN symbolic-ref HEAD 2>/dev/null)")
		if [ "$__GIT_PS1_REV" ]; then
			echo " ($__GIT_PS1_REV)"
		else
			__GIT_PS1_REV=$("$_MR_GIT_BIN" rev-parse HEAD 2>/dev/null | cut -c1-7)
			if [ "$__GIT_PS1_REV" ]; then
				echo " ((${__GIT_PS1_REV}...))"
			fi
		fi
	...
	if [ "$KSH_VERSION" ] || [ "$ZSH_NAME" ] || [ "$BASH_VERSION" ]; then
		...
		alias monorail_color="_MR_CONFIG=$_MR_CONFIG _MR_DIR=$_MR_DIR ""$0"" $_MR_DIR/scripts/color.sh"
		# shellcheck disable=SC2139
		alias monorail_gradient="_MR_CONFIG=$_MR_CONFIG _MR_DIR=$_MR_DIR ""$0"" $_MR_DIR/scripts/gradient.sh"
		# shellcheck disable=SC2139
		alias monorail_image="_MR_CONFIG=$_MR_CONFIG _MR_DIR=$_MR_DIR ""$0"" $_MR_DIR/scripts/image.sh"
		# shellcheck disable=SC2139
		alias monorail_textgradient="_MR_CONFIG=$_MR_CONFIG _MR_DIR=$_MR_DIR ""$0"" $_MR_DIR/scripts/gradient.sh --text"
		# shellcheck disable=SC2139
		alias monorail_rgb="$0"" $_MR_DIR/scripts/rgb.sh"
		# shellcheck disable=SC2139
		alias rgb="ksh $_MR_DIR/scripts/rgb.sh"
		# shellcheck disable=SC2139
		alias monorail_save="_MR_CONFIG=$_MR_CONFIG _MR_DIR=$_MR_DIR ksh $_MR_DIR/scripts/save.sh"
