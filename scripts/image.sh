...
_MAIN() {
	...
	*.jpg | *.jpeg | *.png | *.webp)
		...
		for RGB in $(convert -crop ${WIDTH}x1+0+$((${WIDTH} / 2)) +repage -scale 200x "${XDG_PICTURES_DIR-${HOME}/Pictures}/${THEME}" RGB:- | xxd -ps -c3); do
		...
		rm "${_MONORAIL_CONFIG}/colors.sh"
		{
			...
			[[ ${_PROMPT_TEXT_LUT[0]} ]] && declare -p _PROMPT_LUT
