...
h=${HISTCONTROL:-}
h=${h//ignorespace/}
[[ $h == *"ignoreboth"* ]]&&h=ignoredups:${h//ignoreboth/}
export HISTCONTROL=$h
...
case $_MR_CMD in
c\ *|cd\ *|..*):;;
*)[[ $_MR_DATE ]]||_MR_DATE=$(LC_MESSAGES=C LC_ALL=C date +%m-%d)
case $_MR_DATE in
10-2*|10-3*)ICON=🎃
;;
12*)ICON=🎄
...
case $PWD in
"$HOME")_MR_PWD_BASE=\~;;
*)_MR_PWD_BASE=${NAME-$PWD_BASE}
esac
_MR_TEXT=" $_MR_PWD_BASE$_MR_GIT_PS1 "
_MR_TEXT=${_MR_TEXT//\.\.\./…}
...
_MR_TEXT_FORMATTED=
[[ ${_PROMPT_TEXT_LUT[*]} ]]||_PROMPT_TEXT_LUT[0]="255;255;255"
while [[ $I -lt ${_MR_TEXT_ARRAY_LEN} ]];do
