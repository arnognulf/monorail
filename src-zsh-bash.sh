...
const_color_cursor=21     #discard_for_all
{
	...
	glob__sanitized="${PROMPT_COMMAND:-}"                                                                            #keep_for_bash
	glob__sanitized="${glob__sanitized#"${glob__sanitized%%[![:space:]]*}"}"                                         #keep_for_bash
	glob__sanitized="${glob__sanitized%"${glob__sanitized##*[![:space:]]}"}"                                         #keep_for_bash
	glob__sanitized=${glob__sanitized%;}                                                                             #keep_for_bash
	glob__sanitized=${glob__sanitized#;}                                                                             #keep_for_bash
	glob__prev_cmd=
	[[ $glob__sanitized ]] && PROMPT_COMMAND=("$glob__sanitized")                          #keep_for_bash
	...
	preexec() {
		{
			...
			[[ $glob__has_suffix ]] && glob__suffix
			{
				...
				printf "$var__monorail_title_formatted"
				# shellcheck disable=SC2059 # keep printf compact
				[[ ${glob__colors[const_color_background]} ]] && printf "\e]11;#${glob__colors[const_color_background]}\a\e]10;#${glob__colors[const_color_foreground]}\a\e]12;#${glob__colors[const_color_cursor]}\a\r\e[K"
			} >/dev/tty
	...
	_monorail_gradient() {
		...
		var__hex_cursor_color=$(printf "%.2x%.2x%.2x" "$var__rgb_cur_r" "$var__rgb_cur_g" "$var__rgb_cur_b")
	}
	...
	precmd() {
		{
			if [[ $glob__launched ]]; then
				...
				[[ $BASH_VERSION ]] && history -a >&- #keep_for_bash
...
monorail: warning: Monorail was not found in $MONORAIL_DIR.
				...
				if [[ -z $var__monorail_text_formatted ]]; then
					var__monorail_line=$'\e[4m'
					while [[ $i -lt $COLUMNS ]]; do
						var__monorail_line+=' '
						i=$((i + 1))
					done
					var__monorail_text_formatted=$'\e[0;7m'$var__monorail_text$'\e[0m'
				fi
